...
%phosphorylation rate of phospho-protein(pS6) (rp) is cell cycle-dependent (kcat=rp)
...
K=3 %number of estimated parameters, here 3 ( 3 rp values at G1, S, G2 for active case, rp value is same in G2 and M)
...
[all_init]=create_all_init_pS6(number_of_workers,factor,n_select);


%celldisp(all_init);


% %Load experimental data at 4 min to be fitted
data_1 = readmatrix('pS6_expression_4min_G1.csv');
data_2 = readmatrix('pS6_expression_4min_S.csv');
data_3 = readmatrix('pS6_expression_4min_G2.csv');

%negative data preprocessing on data at 4 min 
...
% x0 =[250.0, 80.0, 1330.0];
% 
% % Lower and upper bounds
% lb = [200.0, 36.0, 1250.0];
% ub = [300.0, 130.0, 1450.0];
% 
% 
% %% ----------------------------------------------------
...
% best_kcat_S = param_est(2);
% best_kcat_G2= param_est(3);
% 
% 
% fprintf('Best parameters:\n kcat = %.4g\n kcat_S = %.4g\n kcat_G2 = %.4g\n', best_kcat, best_kcat_S, best_kcat_G2);
% 
% 
% 
...
% kcat_S=best_kcat_S;
% kcat_G2=best_kcat_G2;
% 
% 
% %% Save to a text file
% fileID = fopen('optimized_parameters_active.txt','w');
% fprintf(fileID, 'Optimized parameters:\n');
% fprintf(fileID, 'kcat   = %.6f\n', kcat);
% fprintf(fileID, 'kcat_S = %.6f\n', kcat_S);
% fprintf(fileID, 'kcat_G2= %.6f\n', kcat_G2);
% fclose(fileID);
% 
% fprintf('Optimized parameters saved to optimized_parameters_active.txt\n');
% ----------------------------------------------------------------------
...
% parallel workers
NL_model_configuration_generator(t_end, initial_cell_no, number_of_workers, kcat, kcat_S, kcat_G2,all_init);
...
function RSS = loss_function(params, t_end, initial_cell_no, number_of_workers, all_init, mean_expt, var_expt, factor)
    ...
    fid = fopen('loss_track_active.txt', 'a');  % append mode
    fprintf(fid, 'kcat=%.4f, kcat_S=%.4f, kcat_G2=%.4f  RSS=%.6f, mean_model=[%.3f %.3f %.3f]\n', ...
        kcat,kcat_S,kcat_G2, RSS, mean_model(1), mean_model(2), mean_model(3));
    fclose(fid);
