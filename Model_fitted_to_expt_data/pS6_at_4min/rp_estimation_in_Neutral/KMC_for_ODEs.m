...
while any(op_m,'all')
    ...
    %options = propensities_from_reactants(rxn_reactants,op_m(:,1:end-1),[rxn_rates; k],state);
    ...
    for i=1:length(op_m(:,1))
        % disp('Size mismatch debug:');
        % disp(['size(second_cell_n_protein): ', mat2str(size(second_cell_n_protein))])
        % disp(['size(op_m cols): ', mat2str(size(op_m(rxn==length(stoich(:,1)),length(rxn_stoich(1,:))+1:end)))])
