...
3. This code is to plot the protein abundances in terms of geometric mean for all timepoints at G1, S and G2-M stages

Note: We did not get disctinct M population here
...
def load_fcs_as_matrix(filename):
 ...
 # print(f'min of  data is {min_data}')
    ...
    for time_idx,time in enumerate(time_strings): #[0,5]
        for protein_idx,name in enumerate(name_list): #[0,53]
            for i in range(len(stage_strings)-1):#2 [0,1] between [G1,S] and [S,G2]
                ...
                # data2[data2<0] = 0 # ( x54)
                #---------------------------------#
                #print(time_idx,protein_idx,stage_strings[i],stage_strings[i+1])
...
def check_increase(data1,data2): # this returns increase or decrease of specific protein between two cell stages
   ...
   # print(decrease, increase)
    ...
    for idx,a in enumerate(increase):#54
        ...
        # print(f'data1={data1[:,idx]}')
        
        # print(f'data2={data2[:,idx]}')
        # print(np.shape(data2))
    ...
    # print(p_decrease)
    # print(p_increase)
    comparison_result = mean1<mean2 # it should consist the same result as increase but with significance
...
if __name__=='__main__':
    ...
    #path='./data/without_cisplatin/primary_NK/live_cells/NKG2D_' #Date Jan27, 2025
    ...
    a,b,c,name_list=analyze_data(path)
    ...
    df_test_idx.to_csv('Primary_NK_NKG2D_colormap_logtransformed_feb17_25.csv', index=False) #skip the previous file and look at the file now

    #df_test_idx.to_csv('colormap_logtransformed_NKG2D_Nov5.csv', index=False) #skip the previous file and look at the file now
    ...
    protein_name = b'DAP12'
    timepoint=0 #(2 , 8 , 16, 32, 64, 256)
