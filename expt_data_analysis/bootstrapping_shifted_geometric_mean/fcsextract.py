...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 25 17:16:07 2024
...
def fcsextract(filename):
    ...
    if data_start == 0 and data_end == 0:
        data_start = int(fcs_vars[b'$BEGINDATA'])
        data_end = int(fcs_vars[b'$ENDDATA'])

    #print("fcs_vars:",fcs_vars[b'$PAR'])
...
def writefcs(fcs_vars,events,fcs_file_name,delimiter=","):
    ...
    for i in range(num_dims):
        info_file.write("%s\t%s\n" % (fcs_vars[b"$P%dN"%(i+1)],fcs_vars.get(b"$P%dS"%(i+1),fcs_vars[b"$P%dN"%(i+1)])))     
